...
class DreamBoothDataset(Dataset):
    """
    Lazy-loading dataset for DreamBooth SD-3 LoRA.
    Keeps only image *paths* or HF-datasets indices in memory and
    opens / augments the image on each __getitem__ call.
    """
    ...
    def __init__(
        ...
        self.custom_instance_prompts = None   # filled later if caption column given

        self.use_hf_dataset = False           # flag for code path in __getitem__

        # ────────────────────────────────────────────────────────────────────
        # 1)  INSTANCE IMAGES / CAPTIONS
        # ────────────────────────────────────────────────────────────────────
        if args.dataset_name is not None:
            ...
            if "json" in args.dataset_name:
                dset = load_dataset("json", data_files=args.dataset_name)
            ...
            dset = dset["train"] if isinstance(dset, dict) and "train" in dset else dset
            col_names = dset.column_names
            ...
            if self.image_column not in col_names:
                raise ValueError(f"--image_column '{self.image_column}' not in {col_names}")

            if args.caption_column is not None:
                if args.caption_column not in col_names:
                    raise ValueError(f"--caption_column '{args.caption_column}' not in {col_names}")
                captions = dset[args.caption_column]
                self.custom_instance_prompts = list(
                    itertools.chain.from_iterable(
                        itertools.repeat(c, repeats) for c in captions
                    )
                )

            self.use_hf_dataset = True
            self.dataset = dset
            self.instance_indices = list(
                itertools.chain.from_iterable(
                    itertools.repeat(i, repeats) for i in range(len(dset))
                )
            )
            self.num_instance_images = len(self.instance_indices)

        else:
            # ─ Local folder path ───────────────────────────────────────────
            root = Path(instance_data_root)
            if not root.exists():
                raise ValueError(f"Instance image root {root} does not exist.")
            ...
            self.instance_paths = list(
                itertools.chain.from_iterable(itertools.repeat(p, repeats) for p in paths)
            )
        ...
        if class_data_root is not None:
            ...
            self.class_images_path = class_paths[: self.num_class_images]
            self._length = max(self.num_instance_images, self.num_class_images)
        else:
            self.class_data_root = None
            self._length = self.num_instance_images

        print(f"Total sample size: {self._length}")

        # ────────────────────────────────────────────────────────────────────
        # 3)  TorchVision transforms (reuse in __getitem__)
        # ────────────────────────────────────────────────────────────────────
        self.train_resize = transforms.Resize(size, transforms.InterpolationMode.BILINEAR)
        ...
        self.normalize = transforms.Normalize([0.5], [0.5])  # scales to [-1, 1]
    ...
    def _load_and_transform(self, pil_img):
        img = exif_transpose(pil_img)
        if img.mode != "RGB":
            img = img.convert("RGB")
        img = self.train_resize(img)
        if args.random_flip and random.random() < 0.5:
            img = self.train_flip(img)
        if self.center_crop:
            img = self.train_crop(img)
        else:
            y1, x1, h, w = self.train_crop.get_params(img, (self.size, self.size))
    ...
    def __getitem__(self, idx):
        ...
        # ─ Instance image & prompt ────────────────────────────────────────
        if self.use_hf_dataset:
            rec = self.dataset[self.instance_indices[idx]]
            pil_img = rec[self.image_column]
        else:
            pil_img = Image.open(self.instance_paths[idx])

        example["instance_images"] = self._load_and_transform(pil_img)

        if self.custom_instance_prompts:
            example["instance_prompt"] = self.custom_instance_prompts[idx]
        else:
            example["instance_prompt"] = self.instance_prompt

        # ─ Class conditioning image (optional) ───────────────────────────
        if self.class_data_root:
            cls_img = Image.open(self.class_images_path[idx % self.num_class_images])
            example["class_images"] = self._load_and_transform(cls_img)
...
def _encode_prompt_with_clip(
    ...
    dtype = getattr(text_encoder, "module", text_encoder).dtype
    prompt_embeds = prompt_embeds.to(dtype=dtype, device=device)
